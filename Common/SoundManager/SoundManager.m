...
+(SKAction*)actionWithSoundNamed:(NSString*)soundName waitForCompletion:(BOOL)wait
{
	...
	SKAction* action = [SKAction runBlock:^{ [sound play]; }];

	if (wait)
	{
		SKAction* wait = [SKAction waitForDuration:sound.duration];
		action = [SKAction sequence:@[action, wait]];
	}
	return action;
}
